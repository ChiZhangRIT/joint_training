...
def embedding_attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope("embedding_attention_decoder", dtype=dtype) as scope:
    ...
    return rnn_decoder( emb_inp,                    # decoder_inputs
...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope("embedding_attention_seq2seq", dtype=dtype) as scope:
    ...
    encoder_cell = rnn_cell.EmbeddingWrapper(
        encoder_cell, embedding_classes=50000,
        embedding_size=300, initializer=init)

    # encoder_input is actually "decoder_hiddens". In order to get embedding_attention_seq2seq/RNN/EmbeddingWrapper/embedding:0, we input decoder_input as encoder_input (encoder_input is not used later)
    encoder_outputs, encoder_state = rnn.rnn(
        encoder_cell, decoder_inputs, dtype=dtype)
    ...
    if isinstance(feed_previous, bool):
        return embedding_attention_decoder(
              decoder_inputs,
              encoder_inputs,  # was encoder_state
              None,  # we do not use attention
              cell,
              num_decoder_symbols,
              embedding_size,
              num_heads=num_heads,
